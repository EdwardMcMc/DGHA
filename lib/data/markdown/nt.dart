class NT {
  List<String> translations = [
    """# Anti Discrimination Act 1992
## Section/s Applicable:
### Section 21, Discrimination by Refusal to Allow Guide Dog.
[http://www.adc.nt.gov.au/](http://www.adc.nt.gov.au/)

Excerpt from Section 21:
(1) For the purposes of this Act, a person discriminates on the ground of impairment against a person with a visual, hearing or mobility impairment if the person treats the person with the impairment less favourably because the person has a guide dog.

Maximum Fine for Breach of Act:
Fixed penalty of \$500
""",
    """# قانون مكافحة التمييز 1992
## القسم / ثانية قابلة للتطبيق:
### القسم 21 ، التمييز برفض السماح بدليل الكلاب.
[رض التشريعات الكاملة](http://www.adc.nt.gov.au/)

مقتطف من القسم 21:
(1) لأغراض هذا القانون ، يقوم أي شخص بالتمييز على أساس ضعف ضد شخص يعاني من ضعف بصري أو سمع أو تنقل إذا كان الشخص يعامل الشخص المصاب بإعاقة أقل تفضيلًا لأن الشخص لديه كلب مرشد.

الغرامة القصوى لخرق الفعل:
عقوبة ثابتة قدرها 500 دولار
""",
"""＃1992年“反歧视法”
##部分适用：
###第21节，拒绝允许导盲犬的歧视。
[查看完整的法规](http://www.adc.nt.gov.au/)

摘自第21节：
（1）就本法而言，如果某人因患有导盲犬而对患有损害的人不利，则该人就视觉，听觉或行动障碍的人的损害进行歧视。

违反法案的最高罚款：
定额罚款500美元
""",
"""＃1992年“反歧視法”
##部分適用：
###第21節，拒絕允許導盲犬的歧視。
[查看完整的法規](http://www.adc.nt.gov.au/)

摘自第21節：
（1）就本法而言，如果某人因患有導盲犬而對患有損害的人不利，則該人就視覺，聽覺或行動障礙的人的損害進行歧視。

違反法案的最高罰款：
定額罰款500美元
""",
"""# Anti Discrimination Act 1992
# # Seksyon / s Naaangkop:
### Seksyon 21, Diskriminasyon sa pamamagitan ng pagtanggi sa Payagan ang Gabay sa Aso.
[Tingnan ang buong batas](http://www.adc.nt.gov.au/)

Sipi mula sa Seksyon 21:
(1) Para sa mga layunin ng Batas na ito, ang isang tao ay nagpapakilala sa kadahilanan ng kapansanan laban sa isang taong may kapansanan sa visual, pandinig o kadaliang mapakilos kung ang tao ay tinatrato ang taong may kapansanan na hindi gaanong kaaya-aya dahil ang tao ay may isang aso na gabay.

Pinakamataas na Fine para sa paglabag sa Batas:
Nakatakdang parusa ng \$ 500
""",
"""# Antidiskriminierungsgesetz von 1992
## Section / s Anwendbar:
### Abschnitt 21, Diskriminierung durch Verweigerung der Erlaubnis von Blindenhunden.
[Vollständige Gesetzgebung anzeigen](http://www.adc.nt.gov.au/)

Auszug aus § 21:
(1) Im Sinne dieses Gesetzes diskriminiert eine Person aufgrund einer Beeinträchtigung eine Person mit einer Seh-, Hör- oder Mobilitätsbeeinträchtigung, wenn die Person die Person mit der Beeinträchtigung ungünstiger behandelt, weil die Person einen Blindenhund hat.

Höchststrafe für Verstöße:
Fixe Strafe von 500 \$
""",
"""# Νόμος κατά των διακρίσεων του 1992
## Τμήμα / s Εφαρμοστέο:
### Τμήμα 21, Διακρίσεις με άρνηση να επιτρέψουμε το σκυλί-οδηγό.
[Δείτε την πλήρη νομοθεσία](http://www.adc.nt.gov.au/)

Απόσπασμα από το τμήμα 21:
(1) Για τους σκοπούς της παρούσας Πράξης, ένα άτομο εισάγει διακρίσεις λόγω βλάβης σε πρόσωπο με προβλήματα όρασης, ακοής ή κινητικότητας, εάν το άτομο αντιμετωπίζει το άτομο με αναπηρία λιγότερο ευνοϊκά επειδή το άτομο έχει σκύλο οδηγό.

Μέγιστο πρόστιμο για παραβίαση νόμου:
Σταθερή ποινή \$ 500
""",
"""# भेदभाव विरोधी अधिनियम 1992
## धारा / एस लागू:
### धारा 21, गाइड डॉग को अनुमति देने से इनकार।
[पूरा विधान देखें](http://www.adc.nt.gov.au/)

धारा 21 से अंश:
(१) इस अधिनियम के प्रयोजनों के लिए, एक व्यक्ति एक दृश्य, श्रवण या गतिशीलता हानि के साथ एक व्यक्ति के खिलाफ हानि के आधार पर भेदभाव करता है यदि व्यक्ति उस व्यक्ति के साथ दुर्व्यवहार के साथ कम व्यवहार करता है क्योंकि व्यक्ति के पास एक गाइड कुत्ता है।

अधिनियम के उल्लंघन के लिए अधिकतम जुर्माना:
\$ 500 का निश्चित दंड
""",
"""# Anti Discrimination Act 1992
## Sezione / i applicabile:
### Sezione 21, Discriminazione per rifiuto di consentire al cane guida.
[Visualizza la legislazione completa](http://www.adc.nt.gov.au/)

Estratto dalla sezione 21:
(1) Ai fini della presente legge, una persona discrimina in base alla menomazione nei confronti di una persona con una disabilità visiva, uditiva o della mobilità se la persona tratta la persona con la menomazione in modo meno favorevole perché la persona ha un cane guida.

Multa massima per violazione dell'atto:
Penalità fissa di \$ 500
""",
"""# 차별 금지법 1992
적용 가능한 섹션 :
### 섹션 21, 안내견을 허용하기위한 거부에 의한 차별.
[전체 법률보기](http://www.adc.nt.gov.au/)

섹션 21에서 발췌 :
(1)이 법의 목적 상, 안내견이있어 장애가있는 사람을 덜 호의적 인 것으로 취급하는 경우, 시각, 청각 또는 이동 장애가있는 사람에 대해 장애 근거로 차별을합니다.

법 위반에 대한 최대 벌금 :
\$ 500의 고정 된 페널티
""",
"""# विरोधी भेदभाव कानून 1992
## सेक्सन / हरू लागू:
### धारा २१, गाइड कुकुरलाई अनुमति दिन अस्वीकार द्वारा भेदभाव।
[पूर्ण कानून हेर्नुहोस्](http://www.adc.nt.gov.au/)

सेक्सन २१ बाट उद्धरण:
(१) यस ऐनको उद्देश्यका लागि, एक व्यक्ति दृश्य, श्रवणशक्ति वा गतिशीलता कमजोरी भएको व्यक्तिको बिरूद्ध कमजोरीको आधारमा भेदभाव गर्दछ यदि यदि व्यक्तिले कमजोरीको साथ व्यक्तिसँग अनुकूल व्यवहार गर्दछ भने कम व्यक्तिसँग गाईड कुकुर छ।

कानूनको उल्लंघनको लागि अधिकतम फाइनः
Penalty 500 को निश्चित दण्ड
""",
"""# قانون ضد تبعیض 1992
## بخش / ها قابل اجرا:
### بخش 21 ، تبعیض در امتناع از اجازه سگ راهنمای.
[مشاهده قانون کامل](http://www.adc.nt.gov.au/)

گزیده ای از بخش 21:
(1) برای اهداف این قانون ، شخص به دلیل اختلال در بینایی ، شنوایی یا تحرک در صورتی که شخص با اختلال نقص کمتر رفتار کند ، به دلیل اختلال در فرد تبعیض قائل می شود ، زیرا شخص دارای یک سگ راهنما است.

حداکثر جریمه برای نقض قانون:
مجازات ثابت 500 دلار
""",
"""# ਵਿਤਕਰੇ ਵਿਰੋਧੀ ਐਕਟ 1992
## ਭਾਗ / ਸ ਲਾਗੂ
### ਧਾਰਾ 21, ਗਾਈਡ ਕੁੱਤੇ ਦੀ ਆਗਿਆ ਤੋਂ ਇਨਕਾਰ ਦੁਆਰਾ ਵਿਤਕਰਾ.
[مਪੂਰਾ ਕਾਨੂੰਨ ਵੇਖੋ](http://www.adc.nt.gov.au/)

ਸੈਕਸ਼ਨ 21 ਤੋਂ ਅੰਸ਼:
(1) ਇਸ ਐਕਟ ਦੇ ਉਦੇਸ਼ਾਂ ਲਈ, ਇੱਕ ਵਿਅਕਤੀ ਵਿਜ਼ੂਅਲ, ਸੁਣਨ ਜਾਂ ਗਤੀਸ਼ੀਲਤਾ ਦੀ ਕਮਜ਼ੋਰੀ ਵਾਲੇ ਵਿਅਕਤੀ ਦੇ ਵਿਰੁੱਧ ਕਮਜ਼ੋਰੀ ਦੇ ਅਧਾਰ ਤੇ ਵਿਤਕਰਾ ਕਰਦਾ ਹੈ ਜੇ ਉਹ ਵਿਅਕਤੀ ਕਮਜ਼ੋਰ ਵਿਵਹਾਰ ਵਾਲੇ ਵਿਅਕਤੀ ਨਾਲ ਘੱਟ ਅਨੁਕੂਲ ਵਿਵਹਾਰ ਕਰਦਾ ਹੈ ਕਿਉਂਕਿ ਵਿਅਕਤੀ ਕੋਲ ਇੱਕ ਗਾਈਡ ਕੁੱਤਾ ਹੈ.

ਕਾਨੂੰਨ ਦੀ ਉਲੰਘਣਾ ਲਈ ਵੱਧ ਤੋਂ ਵੱਧ ਜੁਰਮਾਨਾ:
\$ 500 ਦਾ ਪੱਕਾ ਜ਼ੁਰਮਾਨਾ
""",
"""# Ley contra la discriminación de 1992
## Sección / s aplicable:
### Sección 21, Discriminación por negarse a permitir perro guía.
[Ver legislación completa](http://www.adc.nt.gov.au/)

Extracto de la Sección 21:
(1) A los efectos de esta Ley, una persona discrimina por motivos de discapacidad contra una persona con discapacidad visual, auditiva o de movilidad si trata a la persona con discapacidad de manera menos favorable porque tiene un perro guía.

Multa máxima por incumplimiento de la ley:
Penalidad fija de \$ 500
""",
"""# Đạo luật chống phân biệt đối xử năm 1992
## Phần / s Áp dụng:
### Phần 21, Phân biệt đối xử bằng cách từ chối cho phép Chó dẫn đường.
[Xem pháp luật đầy đủ](http://www.adc.nt.gov.au/)

Trích từ Phần 21:
.

Mức phạt tối đa cho Vi phạm Đạo luật:
Sửa lỗi phạt \$ 500
""",
  ];
}